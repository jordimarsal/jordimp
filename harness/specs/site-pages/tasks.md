...
- [x] T10: Verification pass — run the full sequence `npx vitest run && npm run check && npm run build`; execute every per-R verification command from `requirements.md` (dist greps per locale, `%PDF` checks, non-locale HTML whitelist, print CSS); write the traceability table in `harness/progress/impl_site-pages.md` (R↔test↔implementation)
