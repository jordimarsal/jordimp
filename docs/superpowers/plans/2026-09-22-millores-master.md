...
2. `- [ ]` F11 — approve spec → implement → review → `done`.
3. `- [ ]` F12 — approve spec → implement → review → `done`.
...
6. `- [ ]` Register the newly approved features in `harness/feature_list.json` (see Task 0).
